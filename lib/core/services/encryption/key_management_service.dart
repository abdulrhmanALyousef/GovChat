...
///   • Supports key rotation and device-change recovery
class KeyManagementService {
  ...
  static Future<void> initializeUserKeys(String uid) async {
    try {
      ...
      // Non-fatal: chat still works, but messages won't encrypt/decrypt.
  ...
  /// Return the current user's private key, or null if none is stored.
  ...
  /// deletes local keys and re-generates so the user gets a fresh pair.
  ///
  /// Old encrypted conversation keys will be inaccessible from the new device.
  ...
  static Future<void> _ensurePublicKeyInFirestore(
    ...
    final snap = await _firestore.collection('users').doc(uid).get();
    if (snap.data()?['publicKey'] != null) return; // already present
